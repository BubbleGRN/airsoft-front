          ...
          <template v-for="item in rents" :key="item._id">
            <v-col cols="12" sm="6" md="4" lg="3" style="display: inline-block; margin-right: 16px">
              <v-card :class="getCardClass(item.returndate)">
                ...
                <v-card-text>
                  <div>
                    <strong>{{ $t('rent.rentdate') }}:</strong> {{ formatDate(item.rentdate) }}
                  </div>
                  <div>
                    <strong>{{ $t('rent.returndate') }}:</strong> {{ formatDate(item.returndate) }}
                  </div>
                  <div>
                    <strong>{{ $t('rent.location') }}:</strong> {{ item.location }}
                  </div>
                </v-card-text>
              </v-card>
            </v-col>
